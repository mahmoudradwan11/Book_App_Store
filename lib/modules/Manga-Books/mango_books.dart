...
class MangaBooks extends StatelessWidget {
  const MangaBooks({Key? key}) : super(key: key);
  @override
    ...
    builder:(context,state){
        ...
        return Scaffold(
          appBar: AppBar(
            ...
            title:const Text('Manga Books'),
            centerTitle: true,
          ),
          body:
              ListView.separated(
                itemBuilder:(context,index)=>builtMangaItem(cubit.mangaBooks[index], context),
                separatorBuilder:(context,index)=>builtDivider(),
                itemCount:cubit.mangaBooks.length,
          ),
        );
    },
    );
  }
  Widget builtMangaItem(MangaModel model, context) => InkWell(
    onTap: (){
