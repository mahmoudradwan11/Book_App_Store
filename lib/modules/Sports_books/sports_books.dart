...
class SportsBooks extends StatelessWidget {
  const SportsBooks({Key? key}) : super(key: key);
  @override
    ...
    return BlocConsumer<BookStoreCubit,BookStoreStates>(
      ...
      builder:(context,state){
        ...
        return Scaffold(
          ...
          body:
          ListView.separated(
            itemBuilder:(context,index)=>builtSportsItem(cubit.sportsBooks[index], context),
            separatorBuilder:(context,index)=>builtDivider(),
            itemCount:cubit.sportsBooks.length,
          ),
        );
      },
    );
  }
  Widget builtSportsItem(SportsModel model, context) => InkWell(
    onTap: (){
